...
export const zh = {
  ...
  'detail.schedule.reuseSession': '复用同一会话',
  ...
  'exec.hint': '执行时生效：工作区决定执行会话落在哪个工作区；模式决定会话的 agent 预设；模型在创建执行会话时钉住（不会改写你的默认模型）；权限经 /permission 命令应用到会话。留空则使用运行时默认。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'detail.schedule.reuseSession': 'Reuse the same session',
  ...
  'exec.hint': 'Applied when the task runs: the workspace decides where the execution session lands; the mode composes the session\'s agent preset; the model is pinned at execution-session creation (it never rewrites your default model); the permission is applied through the /permission command. Blank = runtime default.',
